...
export interface QuizStore extends Readable<Quiz> {
	...
	reset(): void;
	sync(new_items: QuizItem[], new_current_idx: number, new_is_timing: boolean): void;
...
export function createQuizStore(): QuizStore {
	...
	const quiz_data = sessionPersistentStore<Quiz>('quiz-session', createQuiz(dictionary));
