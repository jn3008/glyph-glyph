<script lang="ts">
	import { game_config, updateGameConfigAtDepth } from '$/lib/stores/game-config';
	...
	function getModesAtDepth(path: string[]) {
		...
		for (const key of path) current = current.find((mode) => mode.key === key)?.modes || [];
		return current;
	}
	...
	<div class="selections-container">
		{#each Array.from({ length: selected_path.length }, (_, i) => i + 1) as depth}
			{#if getModesAtDepth(selected_path.slice(0, depth)).length > 0}
				<div
					class="sub-selection-container {depth <= visible_depth ? 'visible' : ''}"
